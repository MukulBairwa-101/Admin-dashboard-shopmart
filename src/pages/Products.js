...
const Products = () => {
  ...
  const {activeTheme,products,setProducts,setPath} = appContext;
  ...
  useEffect(() =>{
    ...
    const columns = [
      ...
      const addProduct =(path,editIdent)=>{
        setPath(`/${path}`)
        navigate(path);
      }
    ...
    <div className="flex justify-content cards-container ">
      ...
      <div>
        <button className= {`btn btn-add pointer ${activeTheme.btnClass}  `} onClick={()=>addProduct('addproduct','editInActive')} >Add {activeTheme.btnText}</button>
      </div>
